/****** Object:  View [dbo].[Web_Orders_InvoiceList_old]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE VIEW [dbo].[Web_Orders_InvoiceList_old]
AS
...
FROM         MAS_POL.dbo.AR_Customer INNER JOIN
                      MAS_POL.dbo.AR_InvoiceHistoryHeader AS inh ON MAS_POL.dbo.AR_Customer.CustomerNo = inh.CustomerNo AND 
                      MAS_POL.dbo.AR_Customer.ARDivisionNo = inh.ARDivisionNo INNER JOIN
                      MAS_POL.dbo.AR_InvoiceHistoryDetail AS ind ON inh.InvoiceNo = ind.InvoiceNo AND 
...
WHERE     i.ItemType='1' AND
          ...
          INVOICEDATE >= DATEADD(year, - 2, GETDATE()) AND
          MODULECODE = 'S/O'
