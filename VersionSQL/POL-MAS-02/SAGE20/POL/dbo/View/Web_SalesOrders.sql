/****** Object:  View [dbo].[Web_SalesOrders]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE VIEW [dbo].[Web_SalesOrders]
AS
SELECT       MAS_POL.dbo.SO_SalesOrderHeader.SalesOrderNo
			...
			, IsNull(CASE WHEN CANCELREASONCODE = 'BO' and dbo.PO_Inventory_ETA.RequiredDate is null THEN dbo.PO_Inventory_AvailableComment.AvailableComment ELSE NULL END,'') AS AvailableComment
			, QuantityOrdered
			, UnitPrice
			, ExtensionAmt
			...
			, CASE WHEN MAS_POL.dbo.SO_INVOICEHEADER.SALESORDERNO IS NULL THEN '' ELSE 'x' END AS Invoiced
			 ,Replace(MAS_POL.dbo.SO_SalesOrderHeader.ShipToName,'''','') as ShipTo
			...
			, CASE WHEN IsNumeric(MAS_POL.dbo.SO_SALESORDERHEADER.UDF_NJ_COOP) = 1 AND CHARINDEX('.', MAS_POL.dbo.SO_SALESORDERHEADER.UDF_NJ_COOP) = 0 AND CHARINDEX(',', MAS_POL.dbo.SO_SALESORDERHEADER.UDF_NJ_COOP) = 0
...
FROM            MAS_POL.dbo.SO_SalesOrderHeader INNER JOIN
                         ...
                         dbo.PO_Inventory_ETA ON MAS_POL.dbo.SO_SalesOrderDetail.ItemCode = dbo.PO_Inventory_ETA.ItemCode LEFT OUTER JOIN
						 dbo.PO_Inventory_AvailableComment ON MAS_POL.dbo.SO_SalesOrderDetail.ItemCode = dbo.PO_Inventory_AvailableComment.ItemCode
WHERE        (MAS_POL.dbo.SO_SalesOrderDetail.ItemType = 1) AND (MAS_POL.dbo.SO_SalesOrderDetail.QuantityOrdered > 0) OR
                         (MAS_POL.dbo.SO_SalesOrderDetail.ItemType = 3) AND (MAS_POL.dbo.AR_Salesperson.SalespersonDivisionNo = '00')  AND (MAS_POL.dbo.SO_SalesOrderDetail.ItemCode <> '/C')
