/****** Object:  View [dbo].[IM_PriceCode_TOMORROW]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE VIEW dbo.IM_PriceCode_TOMORROW
AS
WITH MAXDATE AS (SELECT     ItemCode, CustomerPriceLevel, MAX(ValidDate_234) AS ValidDate
                                            FROM         MAS_POL.dbo.IM_PriceCode
                                            WHERE     (ValidDate_234 <=  DATEADD(DAY, 1, GETDATE()))
