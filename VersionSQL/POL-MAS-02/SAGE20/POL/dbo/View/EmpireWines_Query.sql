/****** Object:  View [dbo].[EmpireWines_Query]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE VIEW [dbo].[EmpireWines_Query]
AS
SELECT     MAS_POL.dbo.CI_ITEM.ITEMCODE AS SKU, CASE WHEN MAS_POL.dbo.CI_ITEM.UDF_RESTRICT_MAX > 0 OR
                      ...
                      CASE WHEN dbo.IM_PriceCode_TODAY_FUTURE.ValidDateDescription_234 LIKE '%, %' and dbo.IM_PriceCode_TODAY_FUTURE.CustomerPriceLevel <> 'J' then dbo.IM_PriceCode_TODAY_FUTURE.ValidDateDescription_234 
							WHEN dbo.IM_PriceCode_TODAY_FUTURE.CustomerPriceLevel <> 'J' OR dbo.IM_PriceCode_TODAY_FUTURE.ValidDateDescription_234 like '%B%' then REPLACE(dbo.IM_PriceCode_TODAY_FUTURE.ValidDateDescription_234,',',', ')
							WHEN dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity1 > 4 then CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup1))
							WHEN dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity5 > 0 and dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity4 < 5 then
								CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup1)) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup2)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity1)+1) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup3)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity2)+1) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup4)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity3)+1) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup5)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity4)+1)
							WHEN dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity4 > 0 and dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity3 < 5 then
								CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup1)) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup2)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity1)+1) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup3)) + '/' +
							    ...
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup4)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity3)+1)
							WHEN dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity3 > 0 AND dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity2 < 5 then
								CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup1)) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup2)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity1)+1) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup3)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity2)+1)
							WHEN dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity2 > 0 AND dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity1 < 5 then
								CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup1)) + ', ' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.DiscountMarkup2)) + '/' +
							    CONVERT(varchar, floor(dbo.IM_PriceCode_TODAY_FUTURE.BreakQuantity1)+1)
							else 'J' + dbo.IM_PriceCode_TODAY_FUTURE.ValidDateDescription_234 end AS Pricing, 
                      dbo.IM_PriceCode_TODAY_FUTURE.ValidDate_234 as 'ValidDate', 
...
FROM         MAS_POL.dbo.CI_Item INNER JOIN
                      ...
                      dbo.IM_PriceCode_TODAY_FUTURE ON MAS_POL.dbo.CI_Item.ItemCode = dbo.IM_PriceCode_TODAY_FUTURE.ItemCode LEFT OUTER JOIN
...
WHERE     (MAS_POL.dbo.AP_VENDOR.UDF_VEND_INACTIVE <> 'Y') AND (MAS_POL.dbo.CI_ITEM.CATEGORY2 <> 'Y') AND 
          ...
          and (MAS_POL.dbo.CI_ITEM.UDF_RESTRICT_ALLOCATED <> 'Y') AND dbo.IM_PriceCode_TODAY_FUTURE.PriceCodeRecord=3 AND dbo.IM_PriceCode_TODAY_FUTURE.CustomerPriceLevel = 'Y' and dbo.IM_ItemWarehouse_000.QuantityOnHand+dbo.IM_ItemWarehouse_000.QuantityOnPurchaseOrder>0.04
