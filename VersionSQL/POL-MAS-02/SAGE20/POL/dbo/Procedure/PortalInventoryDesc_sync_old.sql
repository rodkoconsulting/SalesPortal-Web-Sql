/****** Object:  Procedure [dbo].[PortalInventoryDesc_sync_old]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[PortalInventoryDesc_sync_old]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INSERT PortalInventoryDesc_Current(TimeSync, RepCode, ItemCode, Brand, [Description], Vintage, Uom, BottleSize, DamagedNotes, MasterVendor, RestrictOffSale,
									...
									Upc, ScoreWA, ScoreWS, ScoreIWC, ScoreBH, ScoreVM, ScoreOther, ReceiptDate)
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM PortalInventoryDesc_Previous where RepCode=@RepCode
...
BEGIN
	...
	CASE WHEN MIN(Operation)<>'D' THEN [RestrictMaxCases] ELSE '' END AS RestrictMaxCases,
...
(
  SELECT 'D' as Operation, [RepCode],[ItemCode],[Brand],[Description],[Vintage],[Uom],[BottleSize],[DamagedNotes],[MasterVendor],[RestrictOffSale],
				...
				[ScoreWS],[ScoreIWC], [ScoreBH], [ScoreVM], [ScoreOther], [ReceiptDate]
				 
  FROM dbo.PortalInventoryDesc_Previous
  ...
  SELECT 'I' as Operation, [RepCode],[ItemCode],[Brand],[Description],[Vintage],[Uom],[BottleSize],[DamagedNotes],[MasterVendor],[RestrictOffSale],
				...
				[WineType],[Varietal],[Closure],[Country],[Region],[Appellation],[Organic],[Biodynamic],[SampleFocus],[RestrictBo],[RestrictMo],[Upc],[ScoreWA],
				[ScoreWS],[ScoreIWC], [ScoreBH], [ScoreVM], [ScoreOther],[ReceiptDate]
  FROM dbo.PortalInventoryDesc_Current
  WHERE [RepCode] = @RepCode
) tmp
...
BEGIN
 SELECT
	...
	[ItemCode],[Brand],[Description],[Vintage],[Uom],[BottleSize],[DamagedNotes],[MasterVendor],[RestrictOffSale],
				...
				,CONVERT(varchar, [ReceiptDate], 112) AS ReceiptDate
  FROM PortalInventoryDesc_Current
END
if @@ROWCOUNT>0
BEGIN
DELETE FROM PortalInventoryDesc_Previous where RepCode = @RepCode
INSERT PortalInventoryDesc_Previous(TimeSync, RepCode, ItemCode, Brand,[Description], Vintage, Uom, BottleSize, DamagedNotes, MasterVendor, RestrictOffSale,
									RestrictOnPremise, RestrictOffSaleNotes, RestrictAllocated, RestrictApproval, RestrictMaxCases, RestrictState, 
									RestrictSamples, WineType, Varietal, Closure, Country, Region, Appellation, Organic, Biodynamic, SampleFocus, RestrictBo, RestrictMo,
									Upc, ScoreWA, ScoreWS, ScoreIWC, ScoreBH, ScoreVM, ScoreOther, ReceiptDate)
SELECT
...
WHERE RepCode = @RepCode
END
DELETE FROM PortalInventoryDesc_Current where RepCode = @RepCode
END
