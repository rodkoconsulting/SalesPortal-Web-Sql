/****** Object:  Procedure [dbo].[PortalInventoryQty_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[PortalInventoryQty_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INTO #temp_PortalInventoryQty_Current
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM PortalInventoryQty_Previous where RepCode=@RepCode
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM PortalInventoryQty_Previous where RepCode = @RepCode
INSERT PortalInventoryQty_Previous(TimeSync, RepCode, ItemCode, QuantityAvailable, QtyOnHand, OnSO, OnMO, OnBO)
SELECT
	TimeSync,
	RepCode,
	ItemCode,
