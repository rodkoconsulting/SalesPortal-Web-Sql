/****** Object:  Procedure [dbo].[PortalSampleOrderExport]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[PortalSampleOrderExport]
	@OrderNo bigint
AS
begin try
begin transaction
...
set @cmd = 'E:\BoiCode\2020\sampleportalorderexport2020-handoff.vbs '
...
UPDATE dbo.PortalDebugLog
			SET LastMessage = 'Began Export' where OrderNo = @OrderNo
if @ismasactive = 1
	BEGIN
		UPDATE dbo.PortalSampleOrderTransmitHeader set STATUS='I' WHERE ORDERNO=@OrderNo
		exec xp_cmdshell @cmdline, 'no_output'
		UPDATE dbo.PortalDebugLog
			SET LastMessage = 'Handoff' where OrderNo = @OrderNo
	END
else
	BEGIN
		UPDATE dbo.PortalSampleOrderTransmitHeader set STATUS='Q' WHERE ORDERNO=@OrderNo
	END
SELECT 1
commit transaction
end try
BEGIN CATCH
	...
	@subject='Sales Portal Sample SQL Proc error',
	@body=@ErrorProcedure
	ROLLBACK TRAN
END CATCH
