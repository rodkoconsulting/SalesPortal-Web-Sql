/****** Object:  Procedure [dbo].[dev_PortalOrderDetail_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[dev_PortalOrderDetail_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INTO #temp_dev_PortalOrderDetail_Current
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM dev_PortalOrderDetail_Previous where RepCode=@RepCode
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM dev_PortalOrderDetail_Previous where RepCode = @RepCode
INSERT dev_PortalOrderDetail_Previous(TimeSync, RepCode, SalesOrderNo, LineKey, ItemCode, Quantity, Price, Total, Comment)
SELECT
	TimeSync,
	RepCode,
	[SalesOrderNo],[LineKey],[ItemCode],[Quantity],[Price],[Total],[Comment]
