/****** Object:  Procedure [dbo].[dev_PortalSampleOrderHeader_sync_new]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[dev_PortalSampleOrderHeader_sync_new]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INTO #temp_dev_PortalSampleOrderHeader_Current
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM dev_PortalSampleOrderHeader_Previous where RepCode=@RepCode
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM dev_PortalSampleOrderHeader_Previous where RepCode = @RepCode
INSERT dev_PortalSampleOrderHeader_Previous(TimeSync, RepCode,[PurchaseOrderNo],[OrderRep],ShipToCode,[Date],isPosted)
SELECT
	TimeSync,
	RepCode,
