/****** Object:  Procedure [dbo].[PortalInventoryPrice_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[PortalInventoryPrice_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
SELECT 
	...
	PriceLevel,
	ValidDate
INTO #temp_PortalInventoryPrice_Current
FROM CURRENTPRICE 
WHERE RN < 3
SELECT @TimeSyncPrev = MAX(TimeSync) FROM PortalInventoryPrice_Previous where RepCode=@RepCode
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM PortalInventoryPrice_Previous where RepCode = @RepCode
INSERT PortalInventoryPrice_Previous(TimeSync, RepCode, ItemCode, ContractDescription, PriceLevel, ValidDate)
SELECT 
	TimeSync,
	RepCode,
	ItemCode,
