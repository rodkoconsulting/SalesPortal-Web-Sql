/****** Object:  Procedure [dbo].[dev_PortalOrderHeader_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[dev_PortalOrderHeader_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INTO #temp_dev_PortalOrderHeader_Current
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM dev_PortalOrderHeader_Previous where RepCode=@RepCode
...
(
  ...
  FROM dbo.dev_PortalOrderHeader_Previous
  ...
  FROM #temp_dev_PortalOrderHeader_Current
  WHERE [RepCode] = @RepCode
) tmp
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM dev_PortalOrderHeader_Previous where RepCode = @RepCode
INSERT dev_PortalOrderHeader_Previous(TimeSync, RepCode,[SalesOrderNo],[ARDivisionNo],[CustomerNo],[OrderDate],[ShipExpireDate],[ArrivalDate],[OrderStatus],[HoldCode],[CoopNo],[Comment],ShipTo)
SELECT
	TimeSync,
	RepCode,
