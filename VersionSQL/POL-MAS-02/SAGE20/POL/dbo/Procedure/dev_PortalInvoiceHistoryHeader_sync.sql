/****** Object:  Procedure [dbo].[dev_PortalInvoiceHistoryHeader_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[dev_PortalInvoiceHistoryHeader_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INTO #temp_dev_PortalInvoiceHistoryHeader_Current
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM dev_PortalInvoiceHistoryHeader_Previous where RepCode=@RepCode
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM dev_PortalInvoiceHistoryHeader_Previous where RepCode = @RepCode
INSERT dev_PortalInvoiceHistoryHeader_Previous(TimeSync, RepCode,[InvoiceNo],[HSeqNo],[ARDivisionNo],[CustomerNo],[InvoiceType],[InvoiceDate],[Comment])
SELECT
	TimeSync,
	RepCode,
