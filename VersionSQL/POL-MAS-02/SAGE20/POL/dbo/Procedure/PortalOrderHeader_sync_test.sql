/****** Object:  Procedure [dbo].[PortalOrderHeader_sync_test]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[PortalOrderHeader_sync_test]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INSERT PortalOrderHeader_Current(TimeSync, RepCode, ARDivisionNo, CustomerNo, SalesOrderNo, OrderDate, ShipExpireDate, ArrivalDate, OrderStatus, HoldCode, CoopNo, Comment)
...
WHERE    ((@AccountType = 'REP' and SalespersonNo = @RepCode) or (@AccountType = 'OFF' and SalespersonNo not like 'XX%')) and CAST(ROUND(QuantityOrdered,2) AS FLOAT) > 0
UNION ALL
SELECT     @CurrentDate as TimeSync,
		   @RepCode as RepCode,
		   left(ARDivisionNo,2) as ARDivisionNo,
...
FROM
(
  SELECT 'D' as Operation, [SalesOrderNo],[ARDivisionNo],[CustomerNo],[OrderDate],[ShipExpireDate],[ArrivalDate],[OrderStatus],[HoldCode],[CoopNo],[Comment]
  FROM dbo.PortalOrderHeader_Previous
  WHERE [RepCode] = @RepCode
  UNION ALL
  SELECT 'I' as Operation, [SalesOrderNo],[ARDivisionNo],[CustomerNo],[OrderDate],[ShipExpireDate],[ArrivalDate],[OrderStatus],[HoldCode],[CoopNo],[Comment]
  FROM dbo.PortalOrderHeader_Current
  WHERE [RepCode] = @RepCode
) tmp
 
GROUP BY [SalesOrderNo],[ARDivisionNo],[CustomerNo],[OrderDate],[ShipExpireDate],[ArrivalDate],[OrderStatus],[HoldCode],[CoopNo],[Comment]
...
INSERT PortalOrderHeader_Previous(TimeSync, RepCode,[SalesOrderNo],[ARDivisionNo],[CustomerNo],[OrderDate],[ShipExpireDate],[ArrivalDate],[OrderStatus],[HoldCode],[CoopNo],[Comment])
SELECT
	TimeSync,
	RepCode,
	[SalesOrderNo],[ARDivisionNo],[CustomerNo],[OrderDate],[ShipExpireDate],[ArrivalDate],[OrderStatus],[HoldCode],[CoopNo],[Comment]
FROM PortalOrderHeader_Current
WHERE RepCode = @RepCode
END
