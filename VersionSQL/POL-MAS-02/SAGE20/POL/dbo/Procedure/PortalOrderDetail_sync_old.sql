/****** Object:  Procedure [dbo].[PortalOrderDetail_sync_old]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[PortalOrderDetail_sync_old]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INSERT PortalOrderDetail_Current(TimeSync, RepCode, SalesOrderNo, LineKey, ItemCode, Quantity, Price, Total, Comment)
...
WHERE    TransactionDate > GETDATE() and ((@AccountType = 'REP' and SalespersonNo = @RepCode) or (@AccountType = 'OFF' and SalespersonNo not like 'XX%'))
UNION ALL
SELECT     @CurrentDate as TimeSync,
		   @RepCode as RepCode,
...
(
  ...
  FROM dbo.PortalOrderDetail_Current
  WHERE [RepCode] = @RepCode
) tmp
...
BEGIN
 SELECT
  ...
  FROM PortalOrderDetail_Current
END
if @@ROWCOUNT>0
BEGIN
...
WHERE RepCode = @RepCode
END
DELETE FROM PortalOrderDetail_Current where RepCode = @RepCode
END
