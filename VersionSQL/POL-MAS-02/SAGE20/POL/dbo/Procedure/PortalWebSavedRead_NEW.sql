/****** Object:  Procedure [dbo].[PortalWebSavedRead_NEW]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[PortalWebSavedRead_NEW] 
...
WHERE (PriceLevel <> '') and ((AR_CUSTOMER.ARDIVISIONNO = '00') or (AR_CUSTOMER.ARDIVISIONNO = '02')) and ((@AccountType = 'REP' and AR_Customer.SalespersonNo = @RepCode) or (@AccountType = 'OFF' and AR_Customer.SalespersonNo not like 'XX%'))
	...
	SELECT
		...
		,IsNull(a.PoRequired,'') as 'AcctPo'
		,IsNull(a.Balance,0) as 'AcctBal'
		,IsNull(a.License,'') as 'AcctLic'
		,IsNull(a.Rep,@RepCode) as 'AcctRep'
		,IsNull(a.Suppliers,'') as 'AcctSup'
		,IsNull(a.Buyer1,'') as 'AcctWb1'
		,IsNull(a.Buyer2,'') as 'AcctWb2'
		,IsNull(a.Buyer3,'') as 'AcctWb3'
		,IsNull(a.City,'') as 'AcctCity'
		,IsNull(a.CoopList,'') as 'AcctCoop'
		,IsNull(a.ShipDays,'') as 'AcctDays'
		,IsNull(a.LastOrdered,'') as 'AcctLast'
		,IsNull(a.Premise,'') as 'AcctPrem'
		,IsNull(a.Type,'')  as 'AcctType'
		,IsNull(a.Affil,'')  as 'AcctAffil'
		,IsNull(a.Focus,'')  as 'AcctFocus'
		,IsNull(a.PriceLevel,'')  as 'AcctLevel'
		,IsNull(a.State,'')  as 'AcctState'
		,IsNull(a.County,'')  as 'AcctCounty'
		,IsNull(a.Growth,'') as 'AcctGrowth'
		,IsNull(a.Region,'') as 'AcctRegion'
		,IsNull(a.Status,'') as 'AcctStatus'
		,IsNull(a.Address,'') as 'AcctAddress'
		,IsNull(a.DeliveryNotes,'') as 'AcctDelNote'
		,IsNull(a.CustomerName,'') as 'AcctName'
		,IsNull(a.Notes,'') as 'AcctNote'
		,IsNull(a.Buyer1Email,'') as 'AcctWb1Email'
		,IsNull(a.Buyer1Phone,'') as 'AcctWb1Phone'
		,IsNull(a.Buyer2Email,'') as 'AcctWb2Email'
		,IsNull(a.Buyer2Phone,'') as 'AcctWb2Phone'
		,IsNull(a.Buyer3Email,'') as 'AcctWb3Email'
		,IsNull(a.Buyer3Phone,'') as 'AcctWb3Phone'
	FROM dbo.PortalWebSavedHeader h INNER JOIN dbo.PortalWebSavedDetails d ON h.OrderNo = d.OrderNo
		LEFT OUTER JOIN ACCOUNT_INFO a ON h.CUSTOMERNO = a.CustomerNo
