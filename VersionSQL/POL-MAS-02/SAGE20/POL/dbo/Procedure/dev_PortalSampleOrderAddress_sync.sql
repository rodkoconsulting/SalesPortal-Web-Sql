/****** Object:  Procedure [dbo].[dev_PortalSampleOrderAddress_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[dev_PortalSampleOrderAddress_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INTO #temp_dev_PortalSampleAddresses_Current
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM dev_PortalSampleAddresses_Previous where RepCode=@RepCode
...
(
  ...
  FROM dbo.dev_PortalSampleAddresses_Previous
  ...
  FROM #temp_dev_PortalSampleAddresses_Current
  WHERE [RepCode] = @RepCode
) tmp
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM dev_PortalSampleAddresses_Previous where RepCode = @RepCode
INSERT dev_PortalSampleAddresses_Previous(TimeSync, RepCode, ShipToCode,ShipToRep,ShipToName,ShipToAddress,RepRegion,isUser,isActive)
SELECT
	TimeSync,
	RepCode,
