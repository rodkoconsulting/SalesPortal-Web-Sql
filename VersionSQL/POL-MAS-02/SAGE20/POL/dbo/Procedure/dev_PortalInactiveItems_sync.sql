/****** Object:  Procedure [dbo].[dev_PortalInactiveItems_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[dev_PortalInactiveItems_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
CREATE TABLE #temp_dev_PortalInactiveItems_Current(  
...
IF(@AccountType = 'REP')
BEGIN
INSERT INTO #temp_dev_PortalInactiveItems_Current(TimeSync, RepCode, ItemCode, Brand, [Description], Vintage, Uom, BottleSize, DamagedNotes, MasterVendor, RestrictOffSale,
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM dev_PortalInactiveItems_Previous where RepCode=@RepCode
...
(
  ...
  FROM dbo.dev_PortalInactiveItems_Previous
  ...
  SELECT 'I' as Operation, [RepCode],[ItemCode],[Brand],[Description],[Vintage],[Uom],[BottleSize],[DamagedNotes],[MasterVendor],[RestrictOffSale],
				...
				[WineType],[Varietal],[Closure],[Country],[Region],[Appellation],[Organic],[Biodynamic],[SampleFocus],[RestrictBo],[RestrictMo],[Upc],[ScoreWA],
				[ScoreWS],[ScoreIWC], [ScoreBH], [ScoreVM], [ScoreOther],[ReceiptDate],Regen, [Natural], Vegan, HVE
  FROM #temp_dev_PortalInactiveItems_Current
  WHERE [RepCode] = @RepCode
) tmp
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM dev_PortalInactiveItems_Previous where RepCode = @RepCode
INSERT dev_PortalInactiveItems_Previous(TimeSync, RepCode, ItemCode, Brand,[Description], Vintage, Uom, BottleSize, DamagedNotes, MasterVendor, RestrictOffSale,
									RestrictOnPremise, RestrictOffSaleNotes, RestrictAllocated, RestrictApproval, RestrictMaxCases, RestrictState, 
									RestrictSamples, WineType, Varietal, Closure, Country, Region, Appellation, Organic, Biodynamic, SampleFocus, RestrictBo, RestrictMo,
									Upc, ScoreWA, ScoreWS, ScoreIWC, ScoreBH, ScoreVM, ScoreOther, ReceiptDate,Regen, [Natural], Vegan, HVE)
SELECT
...
WHERE RepCode = @RepCode
END
DROP TABLE #temp_dev_PortalInactiveItems_Current
END
