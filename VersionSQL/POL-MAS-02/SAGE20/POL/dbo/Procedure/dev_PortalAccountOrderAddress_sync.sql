/****** Object:  Procedure [dbo].[dev_PortalAccountOrderAddress_sync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[dev_PortalAccountOrderAddress_sync]
	@UserName varchar(25),
	@TimeSync datetime
AS
BEGIN
...
INTO #temp_dev_PortalAccountAddresses_Current
...
SELECT @TimeSyncPrev = MAX(TimeSync) FROM dev_PortalAccountAddresses_Previous where RepCode=@RepCode
...
if @@ROWCOUNT>0
BEGIN
DELETE FROM dev_PortalAccountAddresses_Previous where RepCode = @RepCode
INSERT dev_PortalAccountAddresses_Previous(TimeSync, RepCode, ARDivisionNo, CustomerNo, ShipToCode, ShipToName, ShipToAddress)
SELECT
	TimeSync
	, RepCode
	, ARDivisionNo
